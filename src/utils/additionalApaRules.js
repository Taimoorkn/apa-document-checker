...
export class AdditionalAPARules {
  constructor() {
    ...
    this.patterns = {
      ...
      supplemental:
        /supplementa[lr](?:y)?\s+(?:material|data|file|information|table|figure)/gi,
    };
  }
  ...
  validateFootnotes(text, structure) {
    ...
    const structuralFootnotes = Array.isArray(structure?.footnotes)
      ? structure.footnotes
      : [];

    const inlineMarkers = text.match(this.patterns.footnote) || [];

    const markerCount = inlineMarkers.length;

    const totalFootnotes = structuralFootnotes.length || markerCount;

    if (totalFootnotes === 0) {
      return issues;
    }

    const heavyCount = structuralFootnotes.length || markerCount;

    if (heavyCount > 3) {
      issues.push({
        title: "Extensive footnote usage",

        description: `Document uses ${heavyCount} footnotes; APA recommends keeping footnotes brief and infrequent`,
        ...
        explanation:
          "Reserve footnotes for material that cannot be integrated smoothly into the main text.",
      });
    }
    ...
    inlineMarkers.forEach((marker) => {
      if (marker.startsWith("[")) {
        issues.push({
          title: "Non-superscript footnote marker",
          ...
          explanation:
            "Use superscript numbers (e.g., 1, 2, 3) instead of bracketed markers like [1].",
        });
      }
    });
  ...
  validateMathematicalEquations(text, structure) {
    ...
    if (this.patterns.equationSymbol.test(text)) {
      ...
      inlineEqs.forEach((eq) => {
        if (eq.length > 50) {
          issues.push({
            ...
            explanation:
              "Display complex equations on separate lines and number them",
          });
        }
      });
      ...
      displayEqs.forEach((eq, index) => {
        ...
        if (!nearbyText.match(/\(\d+\)/)) {
          issues.push({
            ...
            explanation:
              "Number display equations consecutively: (1), (2), etc.",
          });
        }
      });
    }
  ...
  validateLegalReferences(text) {
    ...
    legalCases.forEach((legalCase) => {
      if (!reportedTypes.has("legal-italics")) {
        ...
        const fullCase = text.substring(
          Math.max(0, casePosition - 30),
          casePosition + 50
        );
    ...
    if (legalCitations.length > 0 && !reportedTypes.has("legal-format")) {
      issues.push({
        ...
        explanation:
          "Legal citations should follow standard legal citation format",
      });
      ...
      if (!reportedTypes.has("twitter-citation")) {
        ...
        if (context.includes("Twitter") || context.includes("Tweet")) {
          issues.push({
            ...
            explanation:
              "Format: Author, A. [@username]. (Year, Month Day). Content [Tweet]. Twitter. URL",
          });
    ...
    supplementalRefs.forEach((ref) => {
      ...
      if (!context.match(/see|refer|available|found|included/i)) {
        issues.push({
          ...
          explanation:
            "Specify where supplemental materials can be found (e.g., 'see Supplemental Material online')",
        });
      }
      ...
      if (!ref.match(/Supplement(?:al|ary)\s+(?:Table|Figure)\s+[A-Z]?\d+/)) {
        issues.push({
          ...
          explanation:
            "Label as 'Supplemental Table S1', 'Supplemental Figure S1', etc.",
        });
      }
    });
